...
class CustomWaterWorld(gym.Env):
    def __init__(self, force_fps=True):
        super(CustomWaterWorld, self).__init__()
        self.game = WaterWorld(width=800,height=600)
        self.p = PLE(self.game, fps=30, display_screen=True, force_fps=force_fps)
        self.p.init()
        self.action_space = gym.spaces.Discrete(5)
        self.observation_space = gym.spaces.Box(
            low=0, high=255, shape=(800, 600, 3), dtype=np.uint8
        )
        self.reward_range = (-1, 1)
        self.action_set = [None, 119, 97, 100, 115]
    ...
    def get_action_meanings(self):
        return ["NOOP", "up", "left", "right", "down"]
